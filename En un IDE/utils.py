...
class BBSQueue(Queue):
    ...
    def extend(self, items):
        self.A.extend(items)
        self.A = sorted(self.A,reverse=True, key=lambda item: item.path_cost)
...
class BBSQueueHeuristica(Queue):
    ...
    def extend(self, items):
        self.A.extend(items)
        self.A = sorted(self.A,reverse=True, key=lambda item: item.path_cost+self.gps.h(item))
